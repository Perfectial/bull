...
module.exports = function(Queue) {
  ...
  function getNextMillis(millis, cron, opts) {
    ...
    // check if cron has L or l character for month
    if (cron.toLowerCase().includes('l')) {
      // get days count in current month
      // new Date(2020, 2, 0) is equivalent to new Date(2020, 1, 31)
      var daysInCurrentMonth = new Date(currentDate.getFullYear(), currentDate.getMonth() + 1, 0).getDate();

       // cron template for last date in current month
      var cronTmp = cron.slice(0).replace(/l/gi, daysInCurrentMonth);

      // job running time for last date in current month
      var lastDateInCurrentMonth = getNextInterval(new Date(millis).setDate(daysInCurrentMonth - 1), cronTmp, opts);

      // check if job running time is in past then correct cron template for last date for next month
      if (lastDateInCurrentMonth.getTime() < currentDate.getTime()) {
        // get days count in next month
        // new Date(2020, 2, 0) is equivalent to new Date(2020, 1, 31)
        // month + 2 for getting next month
        var daysInNextMonth = new Date(currentDate.getFullYear(), currentDate.getMonth() + 2, 0).getDate();

        // cron template for last date in next month
        cronTmp = cron.replace(/l/gi, daysInNextMonth);
      }
    }
